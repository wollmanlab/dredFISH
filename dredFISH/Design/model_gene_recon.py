...
class CellTypeNet(nn.Module):
    ...
    def forward(self, X, rnd=False):
        ...
        return fine, coarse, Xrcn, emb, mrgn
    ...
    def fit(self, data_iter, device, lr= 1e-1):
        ...
        for i,data in tqdm(enumerate(data_iter)):
            ...
            # overall loss
            loss_gen= (ctg_lss1 + ctg_lss2)*self.lmd0 + rcn_lss + mrg_lss*self.lmd1 + row_cnst*self.lmd2 + gen_lss*self.lmd3
            loss_gen.backward()
            optimizer_gen.step()
            ...
            if not i%(np.clip(data_iter.n_iter//10, 1, None)):
                ...
                tenx_fine_acc, tenx_crse_acc, tenx_rcn_lss, tenx_mrgn_lss= {}, {}, {}, {}
                for l in data['tenx']:
                    if len(data['tenx'][l]):
                        ...
                        tenx_lgts_fine, tenx_lgts_crse, tenx_ftrs_rcn, tenx_emb, tenx_mrgn= self.forward(tenx_ftrs, rnd=True)
                        ...
                        tenx_rcn_lss[l] = (tenx_ftrs_rcn - tenx_ftrs).square().mean().item()

                smrt_fine_acc, smrt_crse_acc, smrt_rcn_lss, smrt_mrgn_lss= {}, {}, {}, {}
                for l in data['smrt']:
                    if len(data['smrt'][l]):
                        ...
                        smrt_lgts_fine, smrt_lgts_crse, smrt_ftrs_rcn, smrt_emb, smrt_mrgn= self.forward(smrt_ftrs, rnd=True)
                        ...
                        smrt_rcn_lss[l] = (smrt_ftrs_rcn - smrt_ftrs).square().mean().item()

                print('\n'+self.name)
                print('%d\t'%i,
                      '|ttl (ctg1,ctg2,row,mrg): %.2E (%.2E,%.2E,%.2E,%.2E,%.2E)  '%(loss_gen.item(),
                                                                                ctg_lss1.item(),
                                                                                ctg_lss2.item(),
                                                                                rcn_lss.item(),
                                                                                row_cnst.item(),
