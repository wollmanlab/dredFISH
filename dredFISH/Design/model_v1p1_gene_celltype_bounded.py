...
class ReLUX(nn.Module):
    """ReLU with upper bound max
    """
    def __init__(self, max: float=1.0):
        super().__init__()
        self.max = max
        self.scale = 6.0/max 
    
    def forward(self, x):
        return nn.ReLU6(x*self.scale) / self.scale
...
def train_model(trn_dataloader, tst_dataloader, gsubidx, res_path, lmd0, 
    n_bit=24, n_rcn_layers=2, lr=0.1, n_epochs=2, n_iter=2000,
    path_trained_model='',
    disable_tqdm=True,
    ):
    """
    ...
    if isinstance(gsubidx, np.ndarray): gsubidx = torch.from_numpy(gsubidx) 
