...
types_l3tol2 = meta.groupby('subclass_label')['neighborhood_label'].first().loc[types_l3].values
types_l3tol2_code, types_l2_ = pd.factorize(types_l3tol2, sort=True)
assert np.all(types_l2_ == types_l2)

# ctrds_diff = ctrds_l3 - ctrds_l1[types_l3tol1_code]
ctrds_diff = ctrds_l3 - ctrds_l2[types_l3tol2_code]
logging.info(f"cluster diff mat: {ctrds_diff.shape}")

# select to change exc, inh cells only
cond = np.logical_or(types_l3tol1 == 'Glutamatergic', types_l3tol1 == 'GABAergic')
ctrds_diff = ctrds_diff[cond]
logging.info(f"cluster diff mat (exc and inh only): {ctrds_diff.shape}")

Sb = ctrds_diff.T.dot(ctrds_diff)
S = -Sb
k = 24
mus = [0, 1e2, 1e4, 1e6, 1e7]

output = os.path.join(res_dir, "smrt_X_DPNMF_test_v2_excinh_l3tol2.h5ad")
...
output = os.path.join(res_dir, "smrt_X_DPNMF_v2_excinh_l3tol2.h5ad")
