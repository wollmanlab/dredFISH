...
from allen_emb_data_iterators import DataIterCached
...
class CellTypeNet(nn.Module):
    ...
    ----------
        ...
        lmd2: penalty factor on 10X correction
        lmd3: penalty factor on discriminator
    """
    def __init__(self, n_gns, n_cat, lab_map, embmat, reduction= 'max', 
        ...
        self.tenx_embmat= embmat[0]
        self.smrt_embmat= embmat[1]
    ...
    def fit(self, data_iter, lr= 1e-1):
        ...
        for i,data in tqdm(enumerate(data_iter)):
            ...
            tenx_lgts_fine, tenx_lgts_crse, tenx_emb, tenx_mrg_lss= self.forward(tenx_ftrs.mm(self.tenx_embmat))
            ...
            smrt_lgts_fine, smrt_lgts_crse, smrt_emb, smrt_mrg_lss= self.forward(smrt_ftrs.mm(self.smrt_embmat))
            ...
            # fine loss 
            ctg_lss1= (tenx_ctg_lss1 + smrt_ctg_lss1)*0.55
            # coarse loss
            ctg_lss2= (tenx_ctg_lss2 + smrt_ctg_lss2)*0.12
            # forward loss
            mrg_lss= (tenx_mrg_lss + smrt_mrg_lss)*0.33
            # discriminator loss 
            ...
            if self.lmd2:
                wts= self.enc.weight.exp()
                prx= wts / wts.sum() * self.mxpr
                row_cnst= ((prx.sum(1) - data['cnstrnts']).clamp(0)**2).mean()
            ...
            if not i%(data_iter.n_iter//10):
                ...
                for l in data['tenx']:
                    if len(data['tenx'][l]):
                        ...
                        tenx_lgts_fine, tenx_lgts_crse, tenx_emb, tenx_mrgn= self.forward(tenx_ftrs.mm(self.tenx_embmat), rnd=True)
                ...
                for l in data['smrt']:
                    if len(data['smrt'][l]):
                        ...
                        smrt_lgts_fine, smrt_lgts_crse, smrt_emb, smrt_mrgn= self.forward(smrt_ftrs.mm(self.smrt_embmat), rnd=True)
                ...
                print('%d\t'%i,
                      ...
                      '|dsc: %.3f'%(0)#(loss_dsc / nn.BCELoss()(torch.ones(1)/2, torch.ones(1))).item())
                     )
                ...
                learning_crvs[i]= {  'smrt_crse_acc': smrt_crse_acc,
                                     ...
                                     'dsc_lss': 0,#(loss_dsc / nn.BCELoss()(torch.ones(1)/2, torch.ones(1))).item(),
...
def train_model(res_path, lmd1, min_pos):
    ...
    """
    data_iter= DataIterCached(n_iter=2000, cached_path='/bigstore/binfo/mouse/Brain/DRedFISH/Allen_V3_Reference/cache/_3')
    
    path_dict= dict(root_path= '/bigstore/binfo/mouse/Brain/DRedFISH/Allen_V3_Reference',
    tenx_embmat= './10X_dpnmf/weights.csv',
    smrt_embmat= './SmartSeq_dpnmf/weights.csv')
    
    tenx_embmat= torch.FloatTensor(pd.read_csv(os.path.join(path_dict['root_path'], path_dict['tenx_embmat']), index_col=0).values)
    smrt_embmat= torch.FloatTensor(pd.read_csv(os.path.join(path_dict['root_path'], path_dict['smrt_embmat']), index_col=0).values)
    
    model= CellTypeNet(n_gns=     (data_iter.current['tenx_ftrs'].shape[1], data_iter.current['smrt_ftrs'].shape[1]),                      # fixed
                       n_cat=     data_iter.labl_map.shape[1],                                  # fixed
                       lab_map=   data_iter.labl_map,                                           # reasonable val
                       embmat=    (tenx_embmat, smrt_embmat),
                       cnst=      'half_nrml',                                                  # reasonable val
                       reduction= 'max',                                                        # reasonable val
                       n_bit=     24,                                                           # fixed
                       min_pos=   min_pos,                                                      # -- tune
                       lmd1=      lmd1,                                                         # -- tune
                       lmd2=      0,                                                         # reasonable val
                       lmd3=      0,                                                         # reasonable val
...
if __name__=='__main__':
    ...
    n=6
    lmd1= np.random.rand(n)*(lmd1_range[1]-lmd1_range[0]) + lmd1_range[0]
    min_pos= np.random.rand(n)*(min_pos_range[1]-min_pos_range[0]) + min_pos_range[0]
    args= [[res_path, lmd1[i], min_pos[i]] for i in range(n)]
