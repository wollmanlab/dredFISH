...
class InstNrmSimple(nn.Module):
    ...
    def forward(self, Z):
        ...
        # penalty 4 (lower vs higher vs med cells (in batch))
        o = Zlog.sort(0)[0] # sort across rows/cells per col/bit. [0] - val; [1] - indices
        hi_p = o.shape[0]//16 #4
        lo_p = o.shape[0]//4 #4
        a = o[     : lo_p, :] # smaller portion
        b = o[-hi_p:     , :] # bigger portion
        c = o[ lo_p:-hi_p, :] # middle portion

        lo = (a - self.logmin).clamp(0).mean() # above low;  0 if below
        hi = (self.logmax - b).clamp(0).mean() # below high; 0 if above
        md = torch.minimum(
                        (c - self.logmin).clamp(0),
                        (self.logmax - c).clamp(0),
                        ).mean() # penalize the middle, whichever side is closer to the edge
        bit_cnst = lo + hi + md
