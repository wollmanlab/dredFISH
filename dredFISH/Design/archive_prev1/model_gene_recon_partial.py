...
class CellTypeNet(nn.Module):
    ...
    def fit(self, data_iter, device, lr= 1e-1):
        ...
        for i,data in tqdm(enumerate(data_iter)):
            ...
            # overall loss
            loss_gen= rcn_lss + (ctg_lss1 + ctg_lss2)*self.lmd0 + mrg_lss*self.lmd1 + row_cnst*self.lmd2 + gen_lss*self.lmd3
            loss_gen.backward()
            optimizer_gen.step()
            ...
            if not i%(np.clip(data_iter.n_iter//20, 1, None)):
                self.eval()
                ...
                with torch.no_grad():
                    ...
                    print('\n'+self.name)
                    print('%d\t'%i,
                        '|ttl (ctg1,ctg2,mse,row,mrg): %.2E (%.2E,%.2E,%.2E,%.2E,%.2E)  '%(loss_gen.item(),
                                                                                    ctg_lss1.item(),
                                                                                    ctg_lss2.item(),
                                                                                    rcn_lss.item(),
                                                                                    row_cnst.item(),
