...
class InstNrmSimple(nn.Module):
    ...
    def forward(self, X):
        ...
        if self.noise is not None:
            X = X + torch.poisson(self.noise[0]*torch.ones_like(X) + self.noise[1]*torch.randn_like(X))
        X1= X.log10()
        ...
        for i,(ftrs, clsts) in tqdm(enumerate(dataloader), disable=disable_tqdm):
            ...
            if not i%report_freq:
                ...
                with torch.no_grad():
                    ...
                    if i == 0:
                        logging.info(f'|ttl, gene_cnstrnts, sparsity_cnstrnts, (trn vs tst) ctg, rcn, ctg_acc')
                    logging.info(
                        f' {i*batch_size:>5d}/{len(dataloader.dataset):>5d} | '
                        f'{loss_gen.item():.2E}, ' 
                        f'{self.lmd2*row_cnst.item():.2E}, ' 
                        f'{self.lmd3*sparsity_cnst.item():.2E} ('
